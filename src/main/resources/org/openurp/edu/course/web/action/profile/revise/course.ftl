...
<div class="container">
  <style>
    .plain_pre{margin: 0px;padding: 0px;font-family: inherit;font-size:inherit;}
  </style>
  [@b.messages slash="3"/]
  <div class="card card-primary card-outline">
    ...
    <table class="table table-sm table-detail">
      ...
      <tr>
        ...
        <td>[#list course.levels as l]${l.level.name}[#sep],[/#list]</td>
        <td class="title"  width="10%">英文名:</td>
        <td colspan="3">${course.enName!}</td>
      </tr>
      <tr>
  ...
  <div class="card card-primary card-outline">
    ...
    [#if profile??]
      ...
      <tr>
        ...
        <td colspan="3">${profile.prerequisites}</td>
      </tr>
      [/#if]
      <tr>
        <td class="title">教材选用类型:</td>
        <td colspan="3">${profile.bookAdoption}</td>
      </tr>
      ...
      <tr>
        ...
        <td colspan="3">${profile.textbooks}</td>
      </tr>
      [/#if]
      [#if profile.bibliography??]
      <tr>
        <td class="title">参考书目:</td>
        <td colspan="3"><pre class="plain_pre">${profile.bibliography}</pre></td>
      </tr>
      [/#if]
      [#if profile.materials??]
      <tr>
        <td class="title">辅助资料:</td>
        <td colspan="3"><pre class="plain_pre">${profile.materials}</pre></td>
      </tr>
      [/#if]
      [#if profile.majors??]
      <tr>
        <td class="title">适用专业:</td>
        <td colspan="3">${profile.majors!}</td>
      </tr>
      [/#if]
      [#if profile.website??]
      <tr>
        <td class="title">课程网站地址:</td>
        <td colspan="3"><a href="${profile.website}" target="_blank">${profile.website}</a></td>
      </tr>
      [/#if]
    </table>
    [#else]
    ...
    <table class="table table-sm table-mini" style="font-size:13px">
  ...
  <div class="card card-primary card-outline">
    ...
    <div class="card-body" style="padding-top:0px;">
      <table class="table table-sm table-striped table-mini" style="font-size:13px">
       <thead>
         <tr>
           <th width="20%">学年学期</th>
           <th width="22%">开课院系</th>
      ...
      [#list clazzInfos as clazzInfo]
        <tr>
          <td>${clazzInfo.semester.schoolYear}-${clazzInfo.semester.name}</td>
