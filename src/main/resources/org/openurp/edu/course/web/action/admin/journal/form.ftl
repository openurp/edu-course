...
[@b.toolbar title="修改课程"]bar.addBack();[/@]
  [@b.form action=b.rest.save(journal) theme="list" onsubmit="validCreditHour"]
    [@b.field  label="课程"]
       ${journal.course.code!} ${journal.course.name!} ${journal.course.defaultCredits!}
    [/@]
...
<script>
   function validCreditHour(form){
      ...
      [/#list]
      if(sumCreditHours != Number.parseFloat(form['journal.creditHours'].value||'0')){
         alert("分类课时总和"+sumCreditHours+",不等于课程学时"+form['journal.creditHours'].value);
         return false;
      }else{
         return true;
      }
