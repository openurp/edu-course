<style>
  ...
  [@b.form action="!saveDesign" theme="list" onsubmit="validateSection"]
    ...
    [#list 1..10 as sectionIndex]
      ...
      [@b.editor theme="mini" name="sections["+sectionIndex+"].summary" label="教学内容提要" required=required value=(design.getSection(sectionIndex).summary)! style="width:600px" maxlength="4000" allowImageUpload="true" uploadJson="!uploadImage.json?program.id=${program.id}"/]
      [@b.editor theme="mini" name="sections["+sectionIndex+"].details" label="教学过程设计" required=required value=(design.getSection(sectionIndex).details)! style="width:600px" maxlength="4000" allowImageUpload="true" uploadJson="!uploadImage.json?program.id=${program.id}"/]
    [/#list]
